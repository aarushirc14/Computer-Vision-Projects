...
def compute_distances(X1, X2):
    ...
    # Compute sum of squares for each row, reshaped for broadcasting
    X1_sq = np.sum(np.square(X1), axis=1).reshape(-1, 1)  # shape (M, 1)
    X2_sq = np.sum(np.square(X2), axis=1).reshape(1, -1)  # shape (1, N)
    
    # Compute the cross term (dot product)
    cross_term = np.dot(X1, X2.T)  # shape (M, N)
    
    # Compute squared distances using the identity and then take square root
    dists = np.sqrt(X1_sq + X2_sq - 2 * cross_term)
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        closest_y = y_train[k_indices]
        # Count each label's frequency; np.bincount automatically breaks ties in favor of
        # smaller values because np.argmax returns the first maximal occurrence.
...
def split_folds(X_train, y_train, num_folds):
    ...
    # Split the data indices into folds (using array_split preserves order)
    indices = np.arange(N)
    ...
    for i in range(num_folds):
        ...
        val_idx = folds[i]
        # The training indices are all the indices not in the validation fold
        train_idx = np.hstack(folds[:i] + folds[i+1:])
        
        # Populate the preallocated arrays
        X_trains[i] = X_train[train_idx]
        y_trains[i] = y_train[train_idx]
        X_vals[i] = X_train[val_idx]
        y_vals[i] = y_train[val_idx]
